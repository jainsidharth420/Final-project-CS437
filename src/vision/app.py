...
def detect_corners(image):
    ...
    results = model_trained.predict(source=image, conf=0.25, save_txt=True, save=True)
...
# print(detect_corners("IMG_1207.JPG"))
...
# get the move from 2 FEN positions

def get_move(FEN1, FEN2):
    FEN1 = FEN1.split('/')
    FEN2 = FEN2.split('/')
    move = []
    for i in range(8):
        for j in range(8):
            if FEN1[i][j] != FEN2[i][j]:
                if FEN2[i][j] == '1':
                    move.append(FEN1[i][j] + str(8-i))
                else:
                    move.append(FEN1[i][j] + str(8-i) + '-' + FEN2[i][j] + str(8-i))
    return move

def stockfishbestmoves():
    stockfish = Stockfish()
